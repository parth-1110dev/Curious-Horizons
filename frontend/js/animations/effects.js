...
import { isReducedMotion, withMotion } from "./utils.js";
...
// ─── PART 1: KNOWLEDGE CONSTELLATION ────────────────────────────────────────
/**
 * Triggers the Knowledge Constellation effect.
 * Tiny stars emerge, constellation lines connect them, then dissolve.
 *
 * @param {HTMLElement} [anchor] Optional anchor element (effect appears near it)
 */
export function triggerKnowledgeConstellation(anchor) {
  withMotion(() => {
    const canvas = document.createElement("canvas");
    canvas.className = "ch-constellation-canvas";
    ...
    // Generate star positions clustered in the viewport center/top-center
    const cx = anchor
      ? anchor.getBoundingClientRect().left + anchor.offsetWidth / 2
      : window.innerWidth / 2;
    const cy = anchor
      ? anchor.getBoundingClientRect().top + anchor.offsetHeight / 2
      : window.innerHeight * 0.35;

    const spread = Math.min(window.innerWidth, 480);
    ...
    // Constellation connections (nearby pairs only)
    const connections = [];
    for (let i = 0; i < stars.length; i++) {
      for (let j = i + 1; j < stars.length; j++) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) < 160) {
          connections.push({ a: stars[i], b: stars[j], opacity: 0 });
        }
      }
    }

    let rafId;
    let phase = 0; // 0=emerge, 1=connect, 2=hold, 3=dissolve
    let phaseProgress = 0;
    let lineProgress = 0;
    let dissolveProgress = 0;

    const PHASE_DURATIONS = [400, 450, 250, 500]; // ms
    ...
    function draw(timestamp) {
      ...
      if (phase === 0) {
        // Stars emerge
        const t = Math.min(phaseProgress / PHASE_DURATIONS[0], 1);
        stars.forEach((s, i) => {
          s.opacity = Math.min(1, t * (i % 3 === 0 ? 1.6 : 1.2)); // stagger
          drawStar(ctx, s);
        });
        if (t >= 1) { phase = 1; phaseProgress = 0; }

      } else if (phase === 1) {
        // Lines connect
        const t = Math.min(phaseProgress / PHASE_DURATIONS[1], 1);
        stars.forEach(s => drawStar(ctx, s));
        connections.forEach(c => {
          c.opacity = Math.min(1, t * 1.5);
          drawConnection(ctx, c);
        });
        if (t >= 1) { phase = 2; phaseProgress = 0; }

      } else if (phase === 2) {
        // Hold briefly
        stars.forEach(s => drawStar(ctx, s));
        connections.forEach(c => drawConnection(ctx, c));
        if (phaseProgress > PHASE_DURATIONS[2]) { phase = 3; phaseProgress = 0; }

      } else if (phase === 3) {
        // Dissolve everything
        const t = Math.min(phaseProgress / PHASE_DURATIONS[3], 1);
        const fade = 1 - t;
        stars.forEach(s => { s.opacity = fade; drawStar(ctx, s); });
        connections.forEach(c => { c.opacity = fade * 0.5; drawConnection(ctx, c); });
        if (t >= 1) {
    ...
    // Fade the entire canvas in
    gsap.to(canvas, {
      opacity: 1,
      duration: 0.25,
      ease: "power2.out",
      onComplete: () => { rafId = requestAnimationFrame(draw); }
    });
  });
}
...
function drawStar(ctx, s) {
  ...
  if (s.opacity > 0.3) {
    ...
    glow.addColorStop(0, `${GOLD}${s.opacity * 0.35})`);
...
function drawConnection(ctx, c) {
  ctx.beginPath();
  ctx.moveTo(c.a.x, c.a.y);
  ctx.lineTo(c.b.x, c.b.y);
  ctx.strokeStyle = `${GOLD}${c.opacity * 0.3})`;
  ctx.lineWidth = 0.6;
  ctx.stroke();
}
